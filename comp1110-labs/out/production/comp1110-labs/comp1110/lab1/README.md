...
2.  **Fork the comp1110 labs repo.**
    ...
    a repo means to create a fresh copy of the repo where the copy will be owned
    ...
    `Robin Smith/comp1110-labs` (but with your name on the left).  Text below
    will remind you of where this new repo came from (*"Forked from comp1110"*).
...
3.  **Find this document within your labs repo.**
    ...
    If not, you can navigate there via the navigation bar at the left.
    The navigation bar may be pinned (visible) or unpinned (hidden), but in
    either it should be accessible by clicking the naviation button at top left
    (three dark grey horizontal lines).  Click the navigation button a few times
    to see its effect.   Notie a small push-pin icon at the bottom  of the
    navigation bar.  You can use this to pin the navigation bar in place.
    To find your project, use the navigation bar and *either* click on `Projects`,
    which will bring up a projects page, or else click on your
    avatar at the bottom of the navigation bar, and then select the `Personal projects` tab.
    ...
    something like: `https://gitlab.cecs.anu.edu.au/u1234567/comp1110-lab1/tree/master/src/comp1110/lab1`.
4. **Find Open Issues**
 
    One of the items on your navigation bar (on the left of your GitLab page,
    which, if hidden, can be made visible by clicking the three horizontal bars
    ...
    issues in your navigation bar.  You are welcome (and encouraged) to make
...
2. **Clone your labs repo.**
    ...
    *always* the one in GitLab, so to keep everything synchronized, after editing
    ...
    click the little elipsis (...) below the `Test` button.  Press `Alt+Insert`
...
2. You can now close each of the Issues (tasks) that you've completed.  Go to
   your GitLab web page, in the navigation bar click on `Issues` (if the 
   navigation bar is hidden, click on the three horizontal bars at top left).
